...
#no_of_tweets = 10
lmt = 100000
no_of_tweets = db.Labeled_Tweets.find().count()

if __name__ == '__main__':
    while count < no_of_tweets:
        print "Reading documents"
        cursor = db.Labeled_Tweets.find({}, {'text':1}).skip(count).limit(lmt)
        #cursor = db.random.find({}, {'text':1}).skip(count).limit(lmt)
        count += lmt
        print "Cleaning & Inserting documents"
        for record in cursor:
            strng = cleanThis(record['text'])
            #print(strng)
            db.Labeled_Tweets.update({'_id': record['_id']}, {'$set': {'sanitized_text' : strng}})
